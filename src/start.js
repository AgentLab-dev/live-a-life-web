...
export function startGame(root) {
  ...
  function runAction(id) {
    ...
    if (id === "take-bracelet") {
      player = takeBracelet(moveToAction(player, id));
      walkTarget = null;
      persist();
    }
    ...
    if (id === "take-plane") {
      player = takePlane(moveToAction(player, id));
      walkTarget = null;
      persist();
    }
    if (id === "share-plane") {
      const before = player.stickers;
      player = sharePlane(moveToAction(player, id));
      noteCheer(before, player.stickers);
      walkTarget = null;
      persist();
    }
    if (id === "take-shell") {
      player = takeShell(moveToAction(player, id));
      walkTarget = null;
      persist();
    }
    if (id === "share-shell") {
    ...
    if (id === "take-ribbon") {
      player = takeRibbon(moveToAction(player, id));
      walkTarget = null;
      persist();
    }
    if (id === "share-ribbon") {
      const before = player.stickers;
      player = shareRibbon(moveToAction(player, id));
      noteCheer(before, player.stickers);
      walkTarget = null;
      persist();
    }
    if (id === "take-acorn") {
      player = takeAcorn(moveToAction(player, id));
      walkTarget = null;
      persist();
    }
    if (id === "share-acorn") {
